...
let TOPICS = [];          // published lesson summaries (from data/index.json), in curriculum order
const TOPIC_CACHE = new Map();  // id -> full topic JSON, fetched on first visit
...
async function boot() {
  // data/index.json is generated by tools/build-index.py — one small file with
  // just what the cards and search need. Full lessons load when opened.
  const index = await (await fetch("data/index.json" + BUST)).json();
  // Students only ever see lessons marked ready by the teacher
  TOPICS = index.topics.filter((t) => t.published);
...
// Which track does the current route belong to? (null = neither)
function routeTrack(view, param) {
  if (view === "igcse") return "igcse";
  if (view === "modules") return "ap";
  if (view === "module") {
    const m = moduleList().find((m) => moduleSlug(m.name) === param);
    return m ? trackOf(m.topics[0]) : "ap";
  }
  if (view === "topic") {
    const t = TOPICS.find((t) => t.id === param);
    return t ? trackOf(t) : "ap";
  }
  return null;
}

function setActiveNav(view, track) {
  document.querySelectorAll(".nav-link").forEach((a) => {
    const nav = a.dataset.nav;
    let active;
    if (nav === "igcse") active = track === "igcse";
...
function initHeader() {
  ...
  // Skip link: focus the content directly — setting location.hash to "#main"
  // would be swallowed by the hash router and re-render the page.
  ...
  // Collapsed nav menu (☰) on narrow screens
...
function updateExamChip() {
  ...
  const short = document.getElementById("exam-chip-short");
  const days = daysToExam();
  if (days == null) { label.textContent = "Exam date"; short.textContent = ""; }
  else if (days > 1) { label.textContent = `${days} days to exam`; short.textContent = `${days}d`; }
  else if (days === 1) { label.textContent = "Exam tomorrow!"; short.textContent = "1d!"; }
  else if (days === 0) { label.textContent = "Exam day — you've got this!"; short.textContent = "today!"; }
  else { label.textContent = "Exam done 🎉"; short.textContent = "🎉"; }
}
...
function openModal(content) {
  ...
  typeset(modalBody);
  openOverlay(modal, document.getElementById("modal-close"));
}
...
const ppVideoCount = (t) => t.ppVidCount || 0;
...
function statsStrip(topics) {
  ...
  const totalV = topics.reduce(
    (s, t) => s + (t.vidCount || 0) + ppVideoCount(t), 0);
...
function countBadge(t) {
  ...
  if (t.hasLessonHtml) return el("span", "badge", "interactive lesson");
...
function lessonCard(t, i) {
  ...
  const vids = (t.vidCount || 0) + ppVideoCount(t);
...
function heroSection() {
  ...
  // Returning student? Offer a one-tap jump back to their track (no redirect —
  // the homepage stays fully reachable).
  const savedTrack = localStorage.getItem(K_TRACK);
  if (savedTrack === "igcse" || savedTrack === "ap") {
    const cont = el("a", "continue-pill",
      `Continue where you left off — ${savedTrack === "igcse" ? "IGCSE" : "American Pathway"} →`);
    cont.href = savedTrack === "igcse" ? "#/igcse" : "#/modules";
    copy.appendChild(cont);
  }
...
function heroPlanet() {
  ...
  webp.srcset = "assets/mathclicks-icon-260.webp";
...
function aboutSection() {
  ...
  tk.href = "TIKTOK_URL_HERE";  // same placeholder as the footer — real handle pending
...
// ---------- On-demand CSS/JS (same pattern the lessons use for graphlab.js) ----------
...
function loadCssOnce(id, href) {
  return new Promise((resolve, reject) => {
    ...
    if (l) {
      if (l.dataset.loaded) return resolve();
    ...
    l.rel = "stylesheet";
    l.href = href;
    l.addEventListener("load", () => { l.dataset.loaded = "1"; resolve(); });
    l.addEventListener("error", reject);
    document.head.appendChild(l);
  });
}

// ---------- Mental Math Arena (js/game.js, loaded on demand) ----------

async function renderGame() {
  main.replaceChildren();
  main.appendChild(backLink("#/", "← Home"));
  if (!window.MathClicksGame) {
    const note = el("div", "empty-note", "Loading the game…");
    main.appendChild(note);
    try {
      await Promise.all([
        loadCssOnce("game-css", "css/game.css?v=2"),
        loadScriptOnce("game-config-js", "js/game-config.js?v=1")
          .then(() => loadScriptOnce("game-js", "js/game.js?v=1")),
      ]);
    } catch {
      note.textContent = "The game didn't load — please refresh the page.";
      return;
    }
    // Bail out if the student navigated elsewhere while the game downloaded
    if ((location.hash || "#/").slice(2).split("/")[0] !== "game") return;
    note.remove();
  }
...
function wireLesson(root) {
  ...
  // The lesson HTML uses plain <div>s for these toggles — make them reachable
  // and operable by keyboard without touching the content files.
  const keyActivate = (elm) => {
    elm.tabIndex = 0;
  ...
  cards.forEach((card) => {
    const head = card.querySelector(".example-header");
  ...
  if (cards.length) {
    ...
    cards[0].querySelector(".example-header").setAttribute("aria-expanded", "true");
  }
  root.querySelectorAll(".vocab-card").forEach((c) => {
    c.addEventListener("click", () => {
      c.setAttribute("aria-pressed", String(c.classList.toggle("flipped")));
    });
    keyActivate(c);
...
async function renderTopic(id) {
  ...
  if (!meta) return renderModules();

  if (!lessonStatus(meta.id)) setLessonStatus(meta.id, "progress");
  // Remember which track the student is studying, for the home-page shortcut
  try { localStorage.setItem(K_TRACK, trackOf(meta)); } catch {}

  main.replaceChildren();
  main.appendChild(backLink("#/module/" + moduleSlug(meta.curriculumModule), "← " + meta.curriculumModule));

  // The index only has a summary — fetch the full lesson on first visit.
  let t = TOPIC_CACHE.get(id);
  if (!t) {
    const note = el("div", "empty-note", "Loading lesson…");
    main.appendChild(note);
    try {
      t = await (await fetch("data/topics/" + meta.file + BUST)).json();
    } catch {
      note.textContent = "Couldn't load this lesson — check your connection and refresh the page.";
      return;
    }
    TOPIC_CACHE.set(id, t);
    // Bail out if the student navigated elsewhere while the lesson downloaded
    const here = decodeURIComponent(((location.hash || "").split("/"))[2] || "");
    if (here !== id) return;
    note.remove();
  }
  ...
  if (t.lessonHtml) {
    ...
    main.appendChild(box);
    Promise.all([
      loadCssOnce("lesson-embed-css", "css/lesson-embed.css?v=1"),
      fetch("data/" + t.lessonHtml + BUST).then((r) => r.text()),
    ])
      .then(([, html]) => {
        box.innerHTML = html;
  ...
  } else if (t.lessonHtml && (!t.pastPapers || !t.pastPapers.length)) {
    // Point students at the lesson's built-in self-check; lessons with nothing
    // at all get no placeholder section.
    main.appendChild(el("h3", "section-title", "📝 Test your understanding"));
...
function pastPaperCard(paper, it, i) {
  ...
  // Action row: mark-scheme toggle + video toggle (only rendered if a video exists)
  ...
  let vidWrap = null;
  if (it.video && it.video.trim()) {
    const vidBtn = el("button", "pp-btn pp-btn--video", "🎬 Watch explanation ▾");
    vidBtn.type = "button";
    ...
    vidBtn.addEventListener("click", () => {
      ...
      vidBtn.setAttribute("aria-expanded", String(open));
    });
  ...
  if (vidWrap) card.appendChild(vidWrap);
  return card;
}
...
function showQuestion(quiz, holder) {
  ...
  const done = (isCorrect, correctText) => {
    ...
    fb.setAttribute("role", "status");   // screen readers announce the verdict
...
// Move keyboard focus into a panel when it opens, keep Tab inside it, and
// put focus back where it was when the panel closes.
...
function openOverlay(container, first) {
  lastFocused = document.activeElement;
  (first || focusablesIn(container)[0] || container).focus();
  container.addEventListener("keydown", overlayTrap);
}

function closeOverlay(container) {
  container.removeEventListener("keydown", overlayTrap);
...
function openFormulas() {
  ...
  openOverlay(fpanel, document.getElementById("fpanel-close"));
}
...
function openChat() {
  ...
  openOverlay(chat, document.getElementById("chat-input"));
  if (!chatStarted) {
    chatStarted = true;
    botSay(
      "Hi! This is <strong>Mr Hamouda's instant solver</strong> 👋 It can solve equations, " +
      "work out percentages, and explain formulas — automatically, right here in your browser. " +
      "It only does <strong>math</strong> — try one of these:"
    );
    renderChips([
      "Solve 3x + 5 = 20",
      "Solve x^2 - 5x + 6 = 0",
